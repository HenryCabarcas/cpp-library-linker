...
class Libs {
    ...
    getChildren(element) {
        if (!element) { //vscode.window.showInformationMessage('No dependency in empty workspace');
            return Promise.resolve(this.data);
        }
        if (element) {
            return Promise.resolve(this.data); // return Promise.resolve(this.getDepsInPackageJson(path.join(this.workspaceRoot, 'node_modules', data.label, 'package.json')));
        }
        return Promise.resolve(this.data);
    }
}
