...
export function registerConfigCompletion(context: vs.ExtensionContext) {

    const provider1 = vs.languages.registerCompletionItemProvider('plaintext', {

        provideCompletionItems(document: vs.TextDocument, position: vs.Position, token: vs.CancellationToken, context: vs.CompletionContext) {
    ...
    const provider2 = vs.languages.registerCompletionItemProvider(
        'plaintext',
        {
